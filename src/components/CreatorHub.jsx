    ...
    <div>
      ...
      <div className="bg-gradient-to-br from-purple-500/5 to-transparent border border-purple-800/30 rounded-2xl p-6 mb-6">
        <div className="flex items-start justify-between">
            ...
            <div>
              {editing ? (
                <div className="space-y-2">
                  <input value={profileName} onChange={(e) => setProfileName(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-white text-sm w-48 focus:outline-none focus:border-purple-500" placeholder="Display name" />
                  <input value={profileBio} onChange={(e) => setProfileBio(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-white text-sm w-72 focus:outline-none focus:border-purple-500" placeholder="Short bio" />
                </div>
              ) : (
                <>
          ...
          <button onClick={() => setEditing(!editing)} className="text-xs text-purple-400 hover:text-purple-300 border border-purple-800/40 px-3 py-1.5 rounded-lg transition-colors">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {files.slice(0, 8).map((file) => (
              <div key={file.id} className="aspect-square rounded-lg bg-gradient-to-br from-gray-800 to-gray-900 flex flex-col items-center justify-center relative overflow-hidden">
                <span className="text-xl opacity-40">{file.type.startsWith('image/') ? '🖼' : file.type.startsWith('video/') ? '🎬' : '📁'}</span>
