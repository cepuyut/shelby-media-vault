...
function useShelby() {
  ...
  var upload = useCallback(function(file) {
    ...
    try {
      ...
      setLoading(false);
      return Promise.resolve({ blobId: blobId, name: file.name, size: file.size, type: file.type, chunks: chunks, status: 'stored', timestamp: new Date().toISOString() });
    } catch (err) { setError(err.message); setLoading(false); return Promise.reject(err); }
  }, []);

  var download = useCallback(function() { setLoading(true); setError(null); setLoading(false); return Promise.resolve(null); }, []);
