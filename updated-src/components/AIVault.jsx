    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Store and serve AI-generated content via Shelby Media Kit with verifiable provenance
        </p>
      </div>
      ...
      <div className="bg-gradient-to-br from-blue-500/5 to-transparent border border-blue-800/30 rounded-2xl p-6 mb-6">
        ...
        <p className="text-gray-600 text-xs mt-3">
          Content is generated, stored on Shelby with cryptographic commitment, and served with verifiable provenance metadata.
        </p>
      </div>
      ...
      <div className="bg-gray-900/20 border border-gray-800/40 rounded-xl p-5 mb-6">
        ...
        >
          <input
            ...
            className="hidden"
            onChange={(e) => {
              if (e.target.files?.[0]) handleAIUpload(e.target.files[0]);
            }}
          />
          ...
          {[
            {
              ...
              desc: 'Every AI output gets a unique merkle root committed on Aptos blockchain',
            },
            {
              icon: '📋',
              title: 'Generation Metadata',
              desc: 'Model name, prompt, parameters, and timestamp stored alongside the content',
            },
            {
              icon: '✓',
              title: 'Verification',
              desc: 'Anyone can verify the content origin and integrity through the blob explorer',
            },
          ].map((item) => (
        ...
        {aiFiles.length === 0 ? (
          ...
          <div className="space-y-2">
            {aiFiles.map((file) => (
              <div
                key={file.id}
                className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-gray-800/20 transition-colors"
              >
